...
export default function Height() {
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderMove: (_, gestureState) => {
      const newWeight = Math.min(240, Math.max(3, Math.round(weight + gestureState.dx / 5)));
      setWeight(newWeight);
  ...
  return (
    <View className="flex-1 bg-[#232323]">
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }} className="px-6 pt-8 pb-6">
        <Text className="text-3xl font-bold text-[#CEFF00] text-center mb-9">What Is Your Height</Text>

        <View className="flex-row bg-[#CEFF00] rounded-full self-center mb-40">
          <TouchableOpacity
            className={`py-3 px-6 rounded-full ${unit === "ft" ? "bg-white text-black" : "opacity-60"}`}
            onPress={() => setUnit("ft")}
          >
            <Text className="text-lg font-bold text-black">ft</Text>
          </TouchableOpacity>
          <View className="w-px bg-white/20" />
          <TouchableOpacity
            className={`py-3 px-6 rounded-full ${unit === "cm" ? "bg-white text-black" : "opacity-60"}`}
            onPress={() => setUnit("cm")}
          >
            <Text className="text-lg font-bold text-black">cm</Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="absolute bottom-0 w-full mb-10 px-20">
          <Pressable onPress={() => navigation.navigate('(setup)/Goal')}>
            <CustomButton 
              title="Next" 
